import colorsys, pygame as pg, numpy as np
...
def hsv_to_rgb(H, S, V):
    res = colorsys.hsv_to_rgb(H / 360, S / 100, V / 100)
...
def draw_current():
    ...
    if holding_shift_key:
        ...
        C.object.rect.width *= POLYGONS(C.action).ratio
    else:
            ...
            match event.type:
                ...
                case pg.MOUSEBUTTONDOWN | pg.MOUSEBUTTONUP:
                    ...
                    for button in self.buttons:
                        ...
                        break
                case pg.MOUSEWHEEL:
                    increment = 1 if event.precise_y < 0 else -1
...
class Colorpicker:
    def __init__(self, x, y, bar_width, slider_height, padding=10, pick_size=(20, 20)):
        ...
        self.rect = Rect(x, y, bar_width, 3 * slider_height + 3 * padding + pick_size[1])
        self.surf = Surface(self.rect.size)
        self.surf.set_colorkey(Color(COLORKEY))
    ...
    def scroll(self, event, hue_incr=0, sat_incr=0, val_incr=0):
        colorpicker.hue_slider.scroll(event, 18)
        self.change_texture()
        ...
        for e in pg.event.get():
            ...
            if e.type == pg.MOUSEWHEEL:
                if not holding_shift_key:
                    hotbar.changeAction(e)
                if holding_shift_key:
                    colorpicker.scroll(e, hue_incr=18)
        ####################################################################################################################
        #        Rend_xyER GAME HERE        #
        ####################################################################################################################
